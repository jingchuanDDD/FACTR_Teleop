...
def get_transform_by_name(name, size=224):
    ...
    if name == "hamer_preproc":
        return transforms.Compose(
            [
                transforms.Resize((256, 196), antialias=False),
                transforms.Normalize(
                    mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]
                ),
            ]
        ...
        return transforms.Compose(
            [
                transforms.RandomResizedCrop(
                    size=size, scale=(0.2, 1.0), antialias=False
                ),
                transforms.Normalize(
                    mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]
                ),
    ...
    if name == "hamer":
        ...
        return transforms.Compose(
            [   
                # hammer input size, the width is cropped due to bbox
                transforms.RandomResizedCrop(
                    size=(256, 196), scale=(0.9, 1.0), antialias=False
                ),
                transforms.GaussianBlur(kernel_size=kernel_size),
                # hammer uses the standard params for normalization
                transforms.Normalize(
                    mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]
                ),
            ]
